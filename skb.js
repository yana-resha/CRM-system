...
(() => {
  ...
  function createInputContact () {
    ...
    const choiseValueArray = ['Телефон', 'Email',  'Facebook', 'VK', 'Другое'];
    ...
    //     const selectorF = document.getElementsByClassName("input");
    ...
    //     const im = new Inputmask("+7(999)9999999");
    //     im.mask(selector);
    //   }
    // })
  ...
  function createModalWindow (obj) {
    ...
    // titleContainer.classList.add('change-title-container');
    ...
    if (Object.keys(obj).length > 0) {
      ...
      // titleContainer.append(modalTitle);
  ...
  function createRowAndCell (obj, array = []) {
    ...
    function cellContent (object) {
      ...
      const fullName = `${object.surname} ${object.name} ${object.lastName}`;
      cellName.textContent = fullName;
      // делаю вывод данных даты создания и изменения как на макете 
      ...
      cellCreateClient.innerHTML = createDate + '<span class="time-style">' + createTime + '</span>';
      ...
      cellChangeClient.innerHTML = changeDate + '<span class="time-style">' + changeTime + '</span>';
      ...
      if (object.contacts.length > 0) {
        object.contacts.forEach(el => {
          ...
          tooltipHead.classList.add('tooltip-content')
          const tooltipTail = document.createElement('div');
          tooltipTail.classList.add('tooltip-tail')
          tooltip.append(tooltipHead, tooltipTail);
          tooltip.classList.add('tooltip');
          if (el.type.toUpperCase() === 'VK') {
            const vkBtn = document.createElement('button');
            vkBtn.classList.add('contact-btn');
            vkBtn.innerHTML = '<i class="fa fa-vk" aria-hidden="true"></i>';
            contactBtnContainer.append(vkBtn, tooltip);
            tooltipHead.textContent = `${el.type}: ${el.value.substr(8)}`
          }
          if (el.type === 'Facebook') {
            const fbBtn = document.createElement('button');
            fbBtn.classList.add('contact-btn', 'fb-icon');
            fbBtn.innerHTML = '<i class="fa fa-facebook" aria-hidden="true"></i>';
            contactBtnContainer.append(fbBtn, tooltip);
            tooltipHead.textContent = `${el.type}: ${el.value.substr(8)}`
          }

          if (el.type === 'Телефон') {
            const phoneBtn = document.createElement('button');
            phoneBtn.innerHTML = '<i class="fa fa-phone" aria-hidden="true"></i>';
            phoneBtn.classList.add('contact-btn');
            contactBtnContainer.append(phoneBtn, tooltip);
            tooltipHead.textContent = `${el.type}: ${el.value}`
          }

          if (el.type === 'Email') {
            const emailBtn = document.createElement('button');
            emailBtn.innerHTML = '<i class="fa fa-envelope" aria-hidden="true"></i>';
            emailBtn.classList.add('contact-btn');
            contactBtnContainer.append(emailBtn, tooltip);
            tooltipHead.textContent = `${el.type}: ${el.value}`
          }

          if (!(el.type === 'Email') && !(el.type === 'Телефон') && !(el.type === 'Facebook') && !(el.type.toUpperCase() === 'VK')) {
            const anotherContactBtn = document.createElement('button');
            anotherContactBtn.classList.add('contact-btn');
            anotherContactBtn.innerHTML = '<i class="fa fa-user-circle" aria-hidden="true"></i>';
            contactBtnContainer.append(anotherContactBtn, tooltip);
            tooltipHead.textContent = `${el.type}: ${el.value}`
          }
  ...
  function postNewClient (modalWindow) {
    const newObj = {}; 
    const contactSelectInWindow = Array.from(document.getElementsByClassName('select'));
  ...
  async function workCRM () {
    ...
    searchInput.addEventListener('input', () => {
      ...
      let timerToRunSearch = setTimeout(searchIncludes, 3000, searchString);
    })
